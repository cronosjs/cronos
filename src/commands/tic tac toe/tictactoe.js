...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    async function tictactoe(m) {
      ...
      // winner message
      function createWinMessage(symbol) {
        return `${client.myemojis.get("bad_crown")} **${
          winner === message.author.id
            ? message.author.username
            : opponent.username
        }** ${client.myemojis.get(symbol)} won!`;
      }

      let dbWin =
        winner === message.author.id ? message.author.id : opponent.id;
      let dbLose =
      ...
      let ttLose = await player.findOne({
        _id: dbLose,
      });
      ...
      if (won["x"] || won["o"]) {
        if (ttWin) {
          ttWin.play += 1;
          ttWin.win += 1;
          ttWin.ticTacToe.play += 1;
          ttWin.ticTacToe.win += 1;
          await ttWin.save().catch((e) => console.log(e));
        } else {
        ...
        if (ttLose) {
          ttLose.play += 1;
          ttLose.lose += 1;
          ttLose.ticTacToe.play += 1;
          ttLose.ticTacToe.lose += 1;
          await ttLose.save().catch((e) => console.log(e));
        } else {
          let newPlayer = new player({
            _id: dbLose,
            play: 1,
      ...
      // edit message with the winner
      if (won["x"]) {
        return m.edit(createWinMessage("x"), buttons);
      }
      if (won["o"]) {
        return m.edit(createWinMessage("o"), buttons);
      }
