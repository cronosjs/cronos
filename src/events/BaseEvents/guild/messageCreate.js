const { formatArray, formatPerms, readTime } = require("../../../utils/Utils");
...
module.exports = class messageCreate extends Event {
  ...
  async exec(message) {
    ...
    if (message.guild) {
      ...
      data.user = await this.client.findUser({ userID: message.author.id });
    }
    ...
    if (command) {
      ...
      if (timestamps.has(message.author.id)) {
        ...
        if (now < expirationTime) {
          ...
          let tleft = readTime(Math.round(timeLeft.toFixed(2)));

          let emb = new MessageEmbed()
            .setTitle("Hold on Mcqueen!")
            .setColor("#CE991D")
            .setDescription(`You can use this command in **${tleft}**!`);
      ...
      try {
        ...
        console.log(err);
        return message
          .reply(`Wait, it seems that my devs spilled coffee on the computer`)
          .then((msg) => {
